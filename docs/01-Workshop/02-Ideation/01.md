...
!!! info "RESPONSE: Model responds with a message saying it cannot read the PDF (via Playground)"

### 2.1 Iterate: Copy Data

- Copy the problem directly into the prompt: 
- Note: this uses model context window
- **What do you observe?**
...
### 2.2 Accuracy vs Cost

   1. You may see a response like this.
   1. **Note the Cost in tokens** e.g., `2477` tokens
   1. **Did the response meet your request?** e.g., is it visual?
   1. **Can you improve this?** e.g., get it to draw the picture?
